...
public class SAICSmartAICar : MonoBehaviour {
	...
	public float engineTorque = 750.0f; //驱动扭矩
	public float brakeTorque = 1000f;  //制动扭矩
	public float minimumEngineRPM = 1000.0f; //最小转速min
	public float maximumEngineRPM = 6000.0f; //最大转速min
	public float maximumSpeed = 180.0f; //最大速度
	[HideInInspector]
	...
	public void SmokeInit (){
		...
		for(int i = 0; i < allWheelColliders.Length; i++){
			...
			//ps.GetComponent<ParticleSystem>().emission.enabled = false;
	...
	void SmokeInstantiateRate () {
		...
		allWheelColliders[i].GetGroundHit(out CorrespondingGroundHit);
			...
			if(Mathf.Abs(CorrespondingGroundHit.sidewaysSlip) > .25f || Mathf.Abs(CorrespondingGroundHit.forwardSlip) > .5f){
				if(!wheelParticles[i].emission.enabled && speed > 1)
                // em = wheelParticles[i].emission;
                //em.enabled = true;
                wheelParticles[i].enableEmission = true;
			}else{
				if(wheelParticles[i].emission.enabled)
